...
{
  musicBoughts(first: 5, where: { buyer: "${account}" }) {
    id
    buyer
    nftAddress
    tokenId
...
`
    ...
    const { data, loading, error, refetch } = useQuery(GET_USER_MUSICS)
    ...
    const getOwnerTracks = async () => {
        ...
        setRender("grate")
        if (!loading) {
          data.musicBoughts.forEach(res => {
            console.log(res.tokenId)
            updateUI(res.tokenId)
          })
    ...
    }, [isWeb3Enabled, render])
    //set tracks to localStorage
    //use reducers to add the tracts from the graph

    return (
        <nftContext.Provider value={{ tracks, dispatch, courses }}>{children}</nftContext.Provider>
    )
}
